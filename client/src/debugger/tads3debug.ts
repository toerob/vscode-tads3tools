...
import { Tads3Runtime, IRuntimeBreakpoint, FileAccessor, IRuntimeVariable, timeout, IRuntimeVariableType } from './tads3runtime';
import { Subject } from 'await-notify';

/**
 * This interface describes the tads3-debug specific launch attributes
 * (which are not part of the Debug Adapter Protocol).
 * The schema for these attributes lives in the package.json of the tads3-debug extension.
...
export class Tads3DebugSession extends LoggingDebugSession {
	...
	public constructor(fileAccessor: FileAccessor) {
		...
		this._runtime.on('stopOnBreakpoint', () => {
			this.sendEvent(new StoppedEvent('breakpoint', Tads3DebugSession.threadID));
		});
		this._runtime.on('stopOnDataBreakpoint', () => {
			this.sendEvent(new StoppedEvent('data breakpoint', Tads3DebugSession.threadID));
		});
		this._runtime.on('stopOnInstructionBreakpoint', () => {
			this.sendEvent(new StoppedEvent('instruction breakpoint', Tads3DebugSession.threadID));
		});
		this._runtime.on('stopOnException', (exception) => {
			if (exception) {
				this.sendEvent(new StoppedEvent(`exception(${exception})`, Tads3DebugSession.threadID));
			} else {
				this.sendEvent(new StoppedEvent('exception', Tads3DebugSession.threadID));
			}
		});
	...
	protected threadsRequest(response: DebugProtocol.ThreadsResponse): void {
		...
		response.body = {
			threads: [
				new Thread(Tads3DebugSession.threadID, "thread 1")
			]
		};
		this.sendResponse(response);
	}
	...
	private createSource(filePath: string): Source {
		return new Source(basename(filePath), this.convertDebuggerPathToClient(filePath), undefined, undefined, 'tads3-adapter-data');
	}
}
