...
export async function validateTads2Settings() {
  ...
  const compilerResult = await validateCompilerPath(
...
export async function validateCompilerPath(
  compilerPath: string,
  showSuccess = true
) {
  ...
  if (output) {
    ...
    if (versionResult && versionResult.length > 0) {
      ...
      window.showInformationMessage(
        `Compiler path setting ${compilerPath} is valid, version: ${versionResult[1]} `
      );
      return true;
    }
  }
...
export async function validatePreprocessorPath(
  ppPath: string,
  showSuccess = true
) {
  ...
  if (output) {
    if (output.toString().includes("Copyright (C)")) {
      ...
      window.showInformationMessage(
        `Preprocessor path setting ${ppPath} is valid`
      );
      return true;
    }
  }
