...
export class SnippetCompletionItemProvider implements CompletionItemProvider {
	...
	constructor(private context: ExtensionContext) {
		reaction(() => extensionState.isUsingAdv3Lite, (usingAdv3Lite: Boolean) => {
		...
		reaction(() => extensionState.isUsingTads2, () => {
			client.info(`Loading snippets for Tads2`);
			const liberarySnippetfileResourceFileUri = Uri.joinPath(this.context.extensionUri, 'snippets', 'snippets_tads2.json');
			this.populateSnippetsFromFileUri(liberarySnippetfileResourceFileUri);
		})
	}
