...
const errorOrWarningRegExp =
  /((.*?)[(]([0-9]+)[)]:\s*)?([Ee]rror|[Ww]arning):\s*(.*)/;
 ...
 */
export function parseAndPopulateErrors(
  text: string,
  ...
  if (result) {
    const line: number =
      (result[3] ? Number.parseInt(result[3] as string) : 1) - 1 ?? 0;
    if (result.length >= 4) {
      const characterLengthOfIssueLine = textDocument
        .positionAt(line)
        .character.valueOf();
...
export function parseAndPopulateTads2Errors(
  text: string,
  textDocument: TextDocument,
  collection: DiagnosticCollection
) {
  ...
  for (const row of text.split(/\r?\n/)) {
    ...
    if (result) {
      const line: number =
        (result[2] ? Number.parseInt(result[2] as string) : 1) - 1 ?? 0;
      const message = row;
      const characterLengthOfIssueLine = textDocument
        .positionAt(line)
        .character.valueOf();
      ...
      const filename = result[1]
        ? mapToFullPath(result[1])
        : extensionState.getTads2MainFile();
      if (existsSync(filename.fsPath)) {
