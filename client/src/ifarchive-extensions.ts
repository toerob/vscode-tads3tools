...
export async function performLocalExtensionInstallation(extensionCacheDirectory: string, cachedDirs: string[], extensionState) {
	const selections = await window.showQuickPick(cachedDirs, { canPickMany: true, title: `[OFFLINE INSTALLER] (Select a cached library)` });
