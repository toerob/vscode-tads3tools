import { ObjectDeclarationContext, PropertyContext, IdAtomContext, AssignmentStatementContext, FunctionDeclarationContext, CurlyObjectBodyContext, CodeBlockContext, MemberExprContext, GrammarDeclarationContext, MethodContext } from './Tads2Parser';
...
export class Tads2SymbolListener implements Tads2Listener {
	...
	enterProperty(ctx: PropertyContext) {
		...
		if (this.currentObjectSymbol && (name.match('other[Ss]ide|masterObject'))) {
			if (this.currentObjectSymbol) {
				...
				const prop: ExtendedDocumentSymbolProperties | undefined = this.additionalProperties.get(this.currentObjectSymbol);
				if (prop) {
	...
	enterMethod(ctx: MethodContext){
		try {
			...
			if (this.currentObjectSymbol) {
				let detail = ctx.params()?.text? '(' + ctx.params()?.text +')' : 'method';
				...
				const symbol = DocumentSymbol.create(name, detail, kind, range, range, []);
