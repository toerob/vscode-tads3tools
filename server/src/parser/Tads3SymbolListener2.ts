...
import { CompletionItem, CompletionItemKind, DocumentSymbol, Range, SymbolKind } from 'vscode-languageserver';
...
export class T3OutlinerListener implements Tads3Listener {
	...
	enterAssignmentStatement(ctx: AssignmentStatementContext) {
		...
		if (name !== undefined) {
			...
			symbol['objectScope'] = this.currentObjectSymbol;
			symbol['functionScope'] = this.currentFunctionSymbol;
	...
	enterObjectDeclaration(ctx: ObjectDeclarationContext) {
				...
				for (const template of body?.objectBody()?.templateExpr()) {
					if(template?.text) {
						if (template.text.startsWith('->')) {
							...
							symbol.arrowConnection = arrowConnection ? arrowConnection : symbol.arrowConnection;
						} else if(template.text.startsWith('@')) {
							const at = template.text.replace('@', '');
							symbol.at = at ? at : symbol.arrowConnection;

						}
					}
					const shortName = template._singleString?.text;
					symbol.shortName = shortName ? shortName.substr(1, shortName.length - 2) : symbol.shortName;
				}
			}
		...
		if (level > 0) {
			...
			if (lastObjectOneLevelBelow) {
				...
				symbol.parent = lastObjectOneLevelBelow;
			} else {
	...
	enterProperty(ctx: PropertyContext) {
		...
		const stop = (ctx.stop?.line - 1) ?? start;
		...
		symbol['isAssignment'] = isAssignment;
		...
		*/
		...
		if (detail.includes('TravelConnector')) {
			...
			this.callbackToRun = (destination: any) => {
				...
				this.currentObjectSymbol.travelConnectorMap.set(symbol.name, destination);
		...
		if (this.currentInnerObjectSymbol) {
			...
			symbol.parent = this.currentObjectSymbol;
